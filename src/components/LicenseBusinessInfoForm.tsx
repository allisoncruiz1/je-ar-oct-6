  ...
  return <TooltipProvider>
      <form onSubmit={handleSubmit} className="w-full space-y-6">
        ...
        <div className="w-full">
          ...
          <Input id="preferredName" type="text" value={formData.preferredName} onChange={e => updateFormData({
          preferredName: e.target.value
        })} placeholder="Enter your preferred name (optional)" className="mt-2 h-10 text-base md:text-sm placeholder:text-[#858791]" />
        </div>
        ...
        <div className="w-full space-y-4">
          ...
          {formData.isLicensed === 'yes' && <div className="w-full">
              <div className="flex items-center gap-2 mb-3">
                <Label className="text-sm font-medium text-[#0C0F24] leading-none">
              ...
              <Popover>
                <PopoverTrigger asChild>
                  <Button variant="outline" role="combobox" className={`w-full h-10 justify-between text-left font-normal ${formData.licensedStates.length === 0 ? 'text-[#858791]' : 'text-[#0C0F24]'}`}>
                    {formData.licensedStates.length === 0 ? 'Select state(s)...' : `${formData.licensedStates.length} state(s) selected`}
                ...
                <PopoverContent className="w-full p-0" align="start">
                  <Command>
                    ...
                    <CommandList className="max-h-60">
                      <CommandGroup>
                        {US_STATES.map(state => <CommandItem key={state} onSelect={() => {
                      const isSelected = formData.licensedStates.includes(state);
                      const newStates = isSelected ? formData.licensedStates.filter(s => s !== state) : [...formData.licensedStates, state];
                      updateFormData({
                        licensedStates: newStates
                      });
                    }} className="flex items-center gap-2">
                            <Checkbox checked={formData.licensedStates.includes(state)} className="h-4 w-4" />
                            <span>{state}</span>
                          </CommandItem>)}
                      </CommandGroup>
                    </CommandList>
              ...
              {formData.licensedStates.length > 0 && <div className="mt-2 flex flex-wrap gap-1">
                  {formData.licensedStates.map(state => <Badge key={state} variant="secondary" className="text-xs px-2 py-1 flex items-center gap-1">
                      {state}
                      <X className="h-3 w-3 cursor-pointer hover:text-[#A91616]" onClick={() => {
              ...
              {errors.licensedStates && <p className="text-sm text-[#A91616] mt-1">{errors.licensedStates}</p>}
            </div>}
        </div>
        ...
        <div className="w-full space-y-4">
          ...
          {formData.conductBusinessOutsideUS === 'yes' && <div className="w-full">
              <div className="flex items-center gap-2 mb-3">
                <Label className="text-sm font-medium text-[#0C0F24] leading-none">
              ...
              <Popover>
                <PopoverTrigger asChild>
                  <Button variant="outline" role="combobox" className={`w-full h-10 justify-between text-left font-normal ${formData.internationalCountries.length === 0 ? 'text-[#858791]' : 'text-[#0C0F24]'}`}>
                    {formData.internationalCountries.length === 0 ? 'Select country(ies)...' : `${formData.internationalCountries.length} country(ies) selected`}
                    <ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" />
                  </Button>
                </PopoverTrigger>
                <PopoverContent className="w-full p-0" align="start">
                  <Command>
                    ...
                    <CommandList className="max-h-60">
                      <CommandGroup>
                        {COUNTRIES.map(country => <CommandItem key={country} onSelect={() => {
                      const isSelected = formData.internationalCountries.includes(country);
                      const newCountries = isSelected ? formData.internationalCountries.filter(c => c !== country) : [...formData.internationalCountries, country];
                      updateFormData({
                        internationalCountries: newCountries
                      });
                    }} className="flex items-center gap-2">
                            <Checkbox checked={formData.internationalCountries.includes(country)} className="h-4 w-4" />
                            <span>{country}</span>
                          </CommandItem>)}
                      </CommandGroup>
                    </CommandList>
              ...
              {formData.internationalCountries.length > 0 && <div className="mt-2 flex flex-wrap gap-1">
                  {formData.internationalCountries.map(country => <Badge key={country} variant="secondary" className="text-xs px-2 py-1 flex items-center gap-1">
                      {country}
                      <X className="h-3 w-3 cursor-pointer hover:text-[#A91616]" onClick={() => {
              ...
              {errors.internationalCountries && <p className="text-sm text-[#A91616] mt-1">{errors.internationalCountries}</p>}
            </div>}
        </div>
        ...
        <div className="sticky bottom-0 bg-white border-t border-border p-4 mt-6 max-md:p-3 max-md:mt-4">
          <div className="flex items-center justify-between max-md:flex-col max-md:gap-3">
            ...
            <div className="flex gap-3 max-md:w-full max-md:order-1">
              <Button variant="outline" onClick={onSaveResume} aria-label="Save and resume application later" className="max-md:flex-1 max-md:text-sm">
                Save & Resume Later
              </Button>
              <Button type="submit" disabled={!canContinue} aria-label="Continue to next step" className="max-md:flex-1 max-md:text-sm">
                Continue
              </Button>
            </div>
          </div>
